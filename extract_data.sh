...
# Created from template: https://github.com/ralish/bash-script-template
...
function rename() {
    shopt -s globstar
    for file in data/MarkGotham_xml/Bach,_Johann_Sebastian/Chorales/**; do 
        if [[ -f "$file" ]] && [ "${file: -5}" == ".mscz" ]; then
            # dirname=$(dirname $file)
            # filename="$(basename $dirname).mscz"
            # dirname="$(dirname $dirname)/"
            # path="$dirname$filename"
            path="${file/\/short_score/}" # equivalent but faster
            if [ "$path" != "$file" ]; then
                mv "$file" "$path"
            fi
        fi
